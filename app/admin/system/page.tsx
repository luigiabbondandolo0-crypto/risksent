...
function statusColor(h: HealthResult | null, loading: boolean) {
  if (loading) return "bg-slate-400";
  if (!h) return "bg-slate-300";
  if (!h.ok) return "bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.4)]";
  if (h.ms > 1000) return "bg-orange-500 shadow-[0_0_8px_rgba(249,115,22,0.3)]";
    ...
    <div className="space-y-8 pb-16">
      <header className="flex flex-wrap items-end justify-between gap-4">
        ...
        <button
          ...
          className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-medium text-slate-700 transition-all hover:border-slate-300 hover:bg-slate-50"
        >
        ...
        {services.map((svc, i) => {
          ...
          return (
            ...
            >
              ...
              <div>
                <p className={`text-lg font-semibold font-mono ${!svc.result ? "text-slate-400" : svc.result.ok ? "text-emerald-600" : "text-red-600"}`}>
                  {label}
                </p>
                {svc.result && (
              ...
              <button
                ...
                className="mt-auto flex items-center justify-center gap-1.5 rounded-lg border border-slate-200 bg-slate-50 py-2 text-xs font-medium text-slate-600 transition-all hover:border-slate-300 hover:bg-slate-100 disabled:opacity-40"
              >
