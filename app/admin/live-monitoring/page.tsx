...
export default function AdminLiveMonitoringPage() {
  ...
  return (
    <div className="space-y-8">
        ...
        {statusLoading ? (
          ...
          <div className="space-y-3">
            ...
            <div className="text-sm">
              ...
              <span className={status.summary.supabase === "connected" ? "text-emerald-600 font-medium" : "text-red-600 font-medium"}>
                {status.summary.supabase}
              </span>
              {" "}
              <span className="text-slate-700">Trading: </span>
              <span className={status.summary.trading === "connected" ? "text-emerald-600 font-medium" : "text-red-600 font-medium"}>
            ...
            <div className="space-y-1 text-xs font-mono">
              {status.logs.map((log, i) => (
                <div key={i} className="flex items-start gap-2">
                  ...
                  <span className="text-slate-700">{log.source}</span>
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-6">
        <h2 className="text-lg font-semibold text-slate-900 mb-4">What We Track (Live Check)</h2>
        ...
        <button
          ...
          className="rounded-md bg-emerald-500 px-4 py-2 text-sm font-medium text-white hover:bg-emerald-600 disabled:opacity-60 disabled:cursor-not-allowed"
        >
        ...
        {liveCheckError && (
          <div className="mt-4 rounded-md border border-red-200 bg-red-50 p-3">
            <p className="text-sm text-red-700">{liveCheckError}</p>
          </div>
        )}
            ...
            <div className="space-y-3">
              {liveCheck.humanEvents.map((event, i) => (
                <div key={i} className="rounded-md border border-slate-200 bg-slate-50 p-4">
                  <h3 className="text-sm font-semibold text-slate-700 mb-2">{event.title}</h3>
                  <p className="text-xs text-slate-600 mb-2">{event.human}</p>
