          ...
          >
            <div className="mb-5 flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <h2 className="font-display text-lg font-bold text-slate-900">New Session</h2>
              </div>
            ...
            <form onSubmit={(e) => void submit(e)} className="space-y-4">
              ...
              <div className="relative">
                ...
                >
                  ...
                  <ChevronDown className={`h-4 w-4 text-slate-400 transition-transform ${showSymbols ? "rotate-180" : ""}`} />
                </button>
                {showSymbols && (
                  <div className="absolute left-0 right-0 top-full z-20 mt-1 max-h-72 overflow-y-auto rounded-xl border border-slate-200 bg-white shadow-xl">
                    <div className="sticky top-0 border-b border-slate-100 bg-white p-2">
                      <input
                        ...
                        className="w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-1.5 font-mono text-sm text-slate-800 outline-none placeholder:text-slate-400"
                        autoFocus
                      />
                    </div>
                    {filteredGroups.map((g) => (
                      <div key={g.label}>
                        <p className="px-3 pt-2 pb-0.5 font-mono text-[10px] uppercase tracking-wider text-slate-500">{g.label}</p>
                        {g.symbols.map((s) => (
                          <button
                            key={s}
                            type="button"
                            ...
                            className={`flex w-full items-center px-3 py-1.5 font-mono text-sm transition-colors ${
                              s === symbol ? "bg-indigo-50 text-indigo-600" : "text-slate-700 hover:bg-slate-50"
                            }`}
                          >
                            {s}
                          </button>
              ...
              {err && (
                <p className="rounded-lg bg-red-50 px-3 py-2 font-mono text-[12px] text-red-600 border border-red-100">{err}</p>
              )}

              <div className="flex gap-2 pt-1">
                <button type="button" onClick={onClose} className="flex-1 rounded-xl border border-slate-200 py-2.5 font-mono text-sm text-slate-600 transition-colors hover:border-slate-300 hover:bg-slate-50 hover:text-slate-800">
                  Cancel
                </button>
                <button
                  type="submit"
                  disabled={loading}
                  className="flex-1 rounded-xl bg-emerald-500 py-2.5 font-mono text-sm font-semibold text-white transition-all hover:bg-emerald-600 disabled:opacity-50"
                >
